...
def test_multi_script_management():
    contexts = ['prerequest', 'postrequest', 'background'] # getdata and putdata contexts cannot use --script-name
    script_names = ["c", "b", "a"]
    scripts = {}
    for context in contexts:
...
def test_multi_script_management_with_tenant():
    tenant = 'mytenant'
    conn2 = another_user(tenant)
    contexts = ['prerequest', 'postrequest'] # background context cannot use a tenant, getdata and putdata contexts cannot use --script-name
    script_names = ["c", "b", "a"]
    scripts = {}
    for context in contexts:
        for t in ['', tenant]:
